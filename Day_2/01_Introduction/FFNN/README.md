...
- The set of sequences you collected will be called $\mathbf{X}$. Here $\mathbf{x}_i \in \mathbf{X}$ is a single sequence in your dataset, and $\mathbf{X}$ is the set of all sequences you measured in your experiments.
- The experimentally determined activity values can be expressed as $\mathbf{y}_i \in \mathbb{Y}$. Here again the $\mathbf{y}_i$ are individual measurements and $\mathbb{Y}$ is the set of all activity values you measured in your experiments.
...
- $f:\mathbb{X} \rightarrow \mathbb{Y}$, which is read as "a function $f$ that maps my set of sequences $\mathbb{X}$ to a set of activities $\mathbb{Y}$." In this course you will learn how to find the function that maps your protein sequences to the data you observed using artificial neural networks.
...
Each neuron in a hidden layer receives input from all the neurons of the previous layer, adding depth to the processing capability of the network. If we consider a single hidden layer with $m$ neurons, and our input vector $\mathbf{x} = [x_1, x_2, ..., x_n]$, the output of the $j$-th neuron in this layer can be represented as:

$$
h_j = \phi(\mathbf{x}) = \sum_{i=1}^{n} w_{ij} x_i + b_j
$$

Where $w_{ij}$ is the weight connecting the $i$-th input to the $j$-th neuron, and $b_j$ is the bias for the $j$-th neuron. The output of the entire hidden layer, then, is a vector $\mathbf{h} = [h_1, h_2, ..., h_m]$.
